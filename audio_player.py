...
class AudioPlayer:
    ...
    def _play(self):
        while self.__is_play:
            self.__stream.write(self.__sample[self.__frame_num:self.__frame_num + 1].tobytes())
            self.__frame_num += 1
            if self.__frame_num == len(self.__sample) - 1:
                self.__is_play = False
                break
        self.__has_thread = False
    ...
    def get_position(self):
        return self.__frame_num / self.__audio.frame_rate

    def set_position(self, position):
        self.__frame_num = int(position * self.__audio.frame_rate)
